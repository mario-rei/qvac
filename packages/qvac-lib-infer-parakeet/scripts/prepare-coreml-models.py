#!/usr/bin/env python3
"""Convert ONNX models with external data to single-file format for CoreML.
...
Handles both naming conventions:
  - model.onnx + model.onnx_data   (e.g. Parakeet CTC)
  - model.onnx + model.onnx.data   (e.g. Parakeet TDT)

Usage:
    python3 prepare-coreml-models.py <model_dir> [--suffix _coreml]
...
def has_external_data(model_dir, name):
    base = os.path.join(model_dir, name)
    return os.path.exists(base + "_data") or os.path.exists(base + ".data")


def main():
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("model_dir", help="Directory containing .onnx + external data files")
    ...
    for name in sorted(os.listdir(model_dir)):
        if not name.endswith(".onnx"):
            continue
        if not has_external_data(model_dir, name):
            continue
